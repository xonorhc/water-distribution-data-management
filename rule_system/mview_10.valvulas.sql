...
CREATE MATERIALIZED VIEW IF NOT EXISTS :MVSCHEMA.sa_valvulas AS
SELECT
    v.id,
    v.geom,
    tv.tipo,
...
CREATE INDEX ON :MVSCHEMA.sa_valvulas USING gist (geom);

CREATE UNIQUE INDEX ON :MVSCHEMA.sa_valvulas (id);

GRANT SELECT ON :MVSCHEMA.sa_valvulas TO PUBLIC;
