...
CREATE MATERIALIZED VIEW IF NOT EXISTS :MVSCHEMA.sa_medidores_vazao AS
SELECT
    mv.id,
    mv.geom,
    -- mv.id_setor_medicao,
...
CREATE INDEX ON :MVSCHEMA.sa_medidores_vazao USING gist (geom);

CREATE UNIQUE INDEX ON :MVSCHEMA.sa_medidores_vazao (id);

GRANT SELECT ON :MVSCHEMA.sa_medidores_vazao TO PUBLIC;
