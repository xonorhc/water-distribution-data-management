CREATE TABLE IF NOT EXISTS flushing_and_blow_of (
    object_id serial PRIMARY KEY, -- Unique identifier for each feature in the layer
    asset_type smallint REFERENCES asset_type_water_device_flushing_and_blow_off (id) NOT NULL DEFAULT 0, -- Categorization of the asset based on its function or type
    asset_id varchar(64), -- Identifier assigned to the asset for tracking purposes
    name varchar(64), -- Descriptive name or label for the asset
...
CREATE INDEX ON flushing_and_blow_of USING gist (shape);
